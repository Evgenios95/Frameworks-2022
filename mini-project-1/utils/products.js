...
function fetchData(i) {
  return `
  <div class="product ${productList[i].id}" >
  <h1 class="p_title">${productList[i].p_title}</h1>
  <img class="p_image" src="${productList[i].p_image}">
  <h4 class="p_price">$${productList[i].p_price}</h4>
  <button class="itemsToCart"><img src="images/cartAdd.png" alt="Basket displayed here"></button><br><br>
  <a href="pDescription.html" button class="btn btn-light" style="background-color: #cc7511" onclick="saveDetails('${productList[i].p_image}', '${productList[i].p_title}', '${productList[i].p_price}', '${productList[i].p_details}');">Details</button></a> 
  </div> 
  `;
}
